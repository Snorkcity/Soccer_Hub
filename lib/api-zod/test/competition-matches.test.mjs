...
test("prefers verified native finals codes while preserving existing IDs", () => {
  ...
  assert.deepEqual(classifyDriblCompetitionStage("F2#4", "Finals 2#4"), {
    kind: "finals",
    code: "FW2G4",
  ...
  assert.equal(classifyDriblCompetitionStage("F3#5", "Finals 3#5").code, "FW3G5");
...
test("blocks unfamiliar native finals codes instead of guessing from display wording", () => {
  ...
  assert.equal(classifyDriblCompetitionStage("F2#3", "Finals 2#3").kind, "unknown");
  assert.equal(classifyDriblCompetitionStage("F3#4", "Finals 3#4").kind, "unknown");
