...
def build_athletic_report(player_row, player_name):
    ...
    def jump_comment(val):
        ...
        if val >= 50:
            ...
            if val >= 66:
                advice.append("\nTactical – You can confidently use your aerial ability in both attacking and defending set pieces. Target the ball early and position aggressively.")
        else:
            advice.append("\nTraining - Improving your jumping power will benefit you in aerial contests. Plyometric exercises like box jumps, tuck jumps, and squat jumps are ideal for this.")
            if val <= 33:
                advice.append("\nTactical – Focus on your positioning and timing if you’re involved in set piece defending. Winning the duel can be more about anticipation than height.")
        return " ".join(advice)
    ...
    def vertical_explosiveness_comment(val):
        ...
        advice = []
        if val >= 66:
            advice.append("\nTraining – Your vertical explosiveness is elite. Maintain it through reactive jumps, single-leg plyometrics, and low-rep strength work.")
            advice.append("\nTactical – You can create separation in aerial duels or take off sharply in pressing moments. Use your vertical power to win space quickly.")
        elif val <= 33:
            advice.append("\nTraining – Develop your explosiveness with contrast training: squats + jumps, resistance jumps, and med ball throws.")
            advice.append("\nTactical – In close contests, focus on timing and anticipation. Improving jump mechanics and foot positioning can reduce the gap.")
        else:
            advice.append("\nTraining – Your vertical explosiveness is solid. Refine it with low-volume plyometrics and quick jump series (like pogo hops).")
            advice.append("\nTactical – You can hold your own in explosive moments. Be confident taking on physical duels when timing is right.")
        return " ".join(advice)
    ...
    def explosiveness_comment(val):
        ...
        advice = []
        if val <= 33:
            advice.append("\nTraining – Improve your explosiveness with bounding drills, resistance band sprints, and quick box jumps.")
            advice.append("\nTactical – Be proactive with your positioning against more explosive players. Try make the first moveme in the contest to give yourself a head start.")
        elif val >= 66:
            advice.append("\nTraining – Your explosiveness is strong. Focus on maintaining it through reactive drills and occasional power-based exercises.")
            advice.append("\nTactical – You have power off the mark, use it to explode past opponents in attacking 1v1s or to close the ball in pressing moments.")
        else:
            advice.append("\nTraining – You have a solid explosiveness base. Maintain it with reactive power work like med ball throws and plyometric bounds.")
            advice.append("\nTactical – You have a dependable burst of speed. It can be used in close contests to pass or catch opponents.")
        return " ".join(advice)
...
🦘 Jumping Ability
 • Vertical Total: {round(vertical_total)}% – {band(vertical_total)} {jump_comment(vertical_total)}
\n• Vertical Relative: {round(vertical_relative)}% – {band(vertical_relative)} {vertical_explosiveness_comment(vertical_relative)}
\n• Horizontal: {round(horizontal)}% – {band(horizontal)} {explosiveness_comment(horizontal)}

🌀 Agility
  • Balsom: {round(balsom)}% – {band(balsom)} {agility_comment(balsom)}

⚡ Sprint Profile
  • 0–10: {round(split_0_10)}% {band_sprint(split_0_10)}, 10–20: {round(split_10_20)}% {band_sprint(split_10_20)}, 20–30: {round(split_20_30)}% {band_sprint(split_20_30)} {sprint_comment(split_0_10, split_10_20, split_20_30)}
