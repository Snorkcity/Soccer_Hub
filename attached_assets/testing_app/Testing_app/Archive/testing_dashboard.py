...
app.layout = html.Div([
    ...
    html.Label(
    "Filter by Tier (based on 20–30 split). The final split reflects late-phase speed: who holds form, accelerates, or fades.\n",
    style={"color": "white", "fontWeight": "bold", "fontFamily": "Arial", "marginBottom": "5px"}
    ),
...
def update_athletic_testing_chart(selected_metric, high_clicks, low_clicks):
    ...
    # Metrics where LOWER raw/percentile = better (we normalise to higher=better)
    reverse_metrics = {"Balsom", "0-10 split", "10-20 split", "20-30 split"}

    # Detect if Balsom is a raw time (seconds) rather than percentile
    is_balsom_raw = (
        selected_metric == "Balsom"
        and df[selected_metric].notna().any()
        and df[selected_metric].max() < 25
    )

    # Normalised percentile: ALWAYS higher = better
    if is_balsom_raw:
        mn, mx = df[selected_metric].min(), df[selected_metric].max()
        spread = (mx - mn) if (mx - mn) != 0 else 1.0
        df["norm_pct"] = 100 * (mx - df[selected_metric]) / spread  # lower sec = better
        title_suffix = "(Normalised from raw – lower = better)"
        raw_label = "Time (s)"
    else:
        if selected_metric in reverse_metrics:
            df["norm_pct"] = 100 - df[selected_metric]
            title_suffix = "(Normalised – lower raw = better)"
        else:
            df["norm_pct"] = df[selected_metric]
            title_suffix = "(Normalised – higher raw = better)"
        raw_label = "Percentile"

    # Buckets + colours from normed percentile
    def tier_bucket(v):
        if pd.isna(v) or float(v) == 0: return "N/A"
        v = float(v)
    ...
    def color_from_norm(v):
        if pd.isna(v) or float(v) == 0: return "gray"
        v = float(v)
        if v >= 80: return "green"
        if v >= 60: return "lightgreen"
        if v >= 40: return "orange"
        return "red"

    df["Tier"] = df["norm_pct"].apply(tier_bucket)
    df["Color"] = df["norm_pct"].apply(color_from_norm)

    # ---- Training Focus tied to (selected_metric, Tier) ONLY ----
    def training_focus_by_metric_and_tier(metric: str, tier: str) -> str:
        if tier == "N/A":
            return "No data to assess"

        if metric == "Vertical total":
            return {
                "Strong": "Box threat — attack aerials, maintain power",
                "Solid": "Add plyos to lift peak jump",
                "Needs improvement": "Prioritise jump strength/power",
            }[tier]

        if metric in ["Vertical relative", "Horizontal"]:
            return {
                "Strong": "Explosive — maintain with micro-doses",
                "Solid": "Refine plyo rhythm & intent",
                "Needs improvement": "Build force → then fast plyos",
            }[tier]

        if metric == "Balsom":
            return {
                "Strong": "Quick & agile — maintain COD quality",
                "Solid": "Add COD reps; sharper decel/hip turns",
                "Needs improvement": "Prioritise agility & change of direction",
            }[tier]

        if metric in ["0-10 split", "10-20 split", "20-30 split"]:
            return {
                "Strong": "Elite split — maintain mechanics",
                "Solid": "Refine drive phase & posture",
                "Needs improvement": "Acceleration mechanics + power",
            }[tier]

        # Fallback (shouldn't hit)
        return {
            "Strong": "Maintain strengths with micro-doses",
            "Solid": "Targeted development block",
            "Needs improvement": "Foundational strength & technique",
        }[tier]

    # Compute focus fresh each time
    df["Training Focus"] = df["Tier"].apply(lambda t: training_focus_by_metric_and_tier(selected_metric, t))

    # Quick debug so you can see which branch is used
    print("Metric:", selected_metric, "| Focus sample:", df["Training Focus"].head().tolist())

    # Sorting on normed percentile
    if high_clicks > low_clicks:
        df = df.sort_values("norm_pct", ascending=False)
    elif low_clicks > high_clicks:
        df = df.sort_values("norm_pct", ascending=True)
    ...
    fig = px.bar(
        ...
        title=f"Athletic Testing – {selected_metric} {title_suffix}",
        template="none",
    )

    # Dark styling (since template='none')
    fig.update_layout(
        plot_bgcolor="black",
        paper_bgcolor="black",
        font=dict(color="white"),
        yaxis_title="Percentile (normalised, higher = better)",
        xaxis_title=""
    )

    # Hover (keep norm %, show raw value with label, plus tier + focus)
    fig.update_traces(
        hovertemplate=(
            "<b>%{x}</b><br>"
            "Percentile (norm): %{y:.0f}%<br>"
            + raw_label + ": %{customdata[2]}<br>"
            "Performance: %{customdata[0]}<br>"
            "Training Focus: %{customdata[1]}<extra></extra>"
        ),
        customdata=df[["Tier", "Training Focus", selected_metric]]
    )
...
def update_sprint_line_chart(selected_tier):
    ...
    # Tier classification based on FINAL split (20–30)
    ...
    df["Tier"] = df["20-30 split"].apply(classify_tier)
