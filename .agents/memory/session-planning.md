...
- **Diagram Review tool DONE:** /library/review (admin) — drag-a-box crops stored as review_crop jsonb ARRAY (up to 6 boxes = progressions/variations worked in order; canvas coords, applied via SVG viewBox; DiagramWithCrops renders multi-crop as labelled variations everywhere incl. print; readers normalize legacy single-object rows), part pills (warmup/activation/introduction/main/endgame/unusable). Generate has includeActivation (best cosine match among activation-tagged; older teams passing activation, younger ball mastery — mastery diagrams to be uploaded later). Coach reviews in DEV; copy reviews to prod when he says (match by deck position, NOT id; coach-uploads copy by source_file='coach-upload'+title). Coach-uploaded diagrams: POST /library/practices/upload (image data URI + canvas dims + part/tags, pre-reviewed) → chapter "Uploads", ordinal ≥100000 (deck re-imports upsert by ordinal, never collide); rankPractices admits Uploads into any slot their reviewPart allows; auth before 25mb parser; ordinal insert retries on 23505. Coach plans a full slide re-import later (will re-review/clip over time). + coach's sub-category tags (Activations A1–A8, Main Part MP1–MP11, End Games small/medium/big). Generator: unusable never picked, tagged practices locked to their part, unreviewed still eligible; tags shown to the LLM and outrank wording. syncPracticeLibrary ON CONFLICT does NOT touch review columns — reviews survive re-seeds.
