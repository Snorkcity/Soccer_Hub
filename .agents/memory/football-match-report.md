...
- **Scouting variant (Opponent Insights → Match Report sub-tab):** slimmed per-club report from league tables only (no GPS/possession); reuses the same response shape (tiles empty, gps/ballUse null) and the same dark PPTX deck via a `subjectClub` variant. Scouting semantics FLIP: their high scoring share = threat (amber/red), their high conceding share = our opening (green). Half-time score is oriented to the subject club, same as the scoreline.

- Report goals carry optional typeLabel; goalDna.dayInsights (max 3, team route only): partnerships/streaks/head-to-head DNA mix — new optional fields stay OUT of yaml required for saved-jsonb back-compat. Season-mix benchmark box is its own bottom card.
