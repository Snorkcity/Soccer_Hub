...
DT/AT is recorded for every typed open-play goal (SP goals have no DT/AT). The goals/league_goals tables ALSO carry passString, buildupLane (Left/Centre/Right), howPenetrated, assistType, firstTimeFinish, finishType — so sections 5–8 of the framework ARE buildable. Coverage is per-club AND per-league: Belconnen/Croatia/Olympic/Wanderers well coded; several clubs have goal types only (no pass/lane). Coach confirmed future ingested leagues may have only minutes + scorer (no goal types, assists, or metrics at all) — every goal-intel surface must degrade silently to whatever is coded, never error or nag about missing coding. Every read must check its own sample and show counts, not just percentages (a "100%" off 5 goals over-claims).
