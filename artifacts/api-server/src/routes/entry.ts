...
router.post("/entry/match", async (req, res): Promise<void> => {
  ...
  const { leagueMatch, belconnenMatchId } = await db.transaction(async (tx) => {
    ...
    if (isHome || isAway) {
      ...
      possessionSource: b.possession == null ? "unknown" : "official",
      shots: b.shots ?? null,
      shotsSource: b.shots == null ? "unknown" : "official",
      passes: b.passes ?? null,
      passesSource: b.passes == null ? "unknown" : "official",
      oppShots: b.oppShots ?? null,
      oppShotsSource: b.oppShots == null ? "unknown" : "official",
      oppPasses: b.oppPasses ?? null,
      oppPassesSource: b.oppPasses == null ? "unknown" : "official",
      teamId: b.teamId,
      seasonId: b.seasonId,
    }).returning();
